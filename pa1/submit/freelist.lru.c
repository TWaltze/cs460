...
void ShowLRUList()
{
	if (StrategyControl->lruHead == LRUNEXT_END_OF_LIST)
	{
		printf("[EMPTY]\n");
		return;
	}

	BufferDesc *buf = BufferDescriptors + StrategyControl->lruHead;

	while (buf->lruNext != LRUNEXT_END_OF_LIST)
	{
		printf ("[%2d]->", buf->buf_id);
		buf = BufferDescriptors + buf->lruNext;
	}
 ...
 * CS460
 *
 * This will put a buffer into LRU list. This function should be only called by
 * unpinning a buffer. Since it is LRU, we always put at head position. This
 ...
 */
void StrategyLRUEnqueue(volatile BufferDesc *buf)
{
	(BufferDescriptors + StrategyControl->lruTail)->lruNext = buf->buf_id;

    if ((BufferDescriptors + StrategyControl->lruHead)->lruPrev == LRU_NOT_IN_LIST){
        (BufferDescriptors + StrategyControl->lruHead)->lruPrev = LRUPREV_HEAD_OF_LIST;
    } else {
    ...
    // link to prev
    buf->lruPrev = LRUPREV_HEAD_OF_LIST;

    // link to next
    buf->lruNext = StrategyControl->lruTail;
    StrategyControl->lruTail = buf->buf_id;

    if (buf->lruNext != LRUNEXT_END_OF_LIST) {
        Assert((BufferDescriptors + buf->lruNext)->lruPrev == LRUPREV_HEAD_OF_LIST);
        (BufferDescriptors + buf->lruNext)->lruPrev = buf->buf_id;
    }
    */
}
 ...
 * CS460
 */
void StrategyLRUDequeue(volatile BufferDesc *buf)
{
    if (buf->lruNext == LRU_NOT_IN_LIST) {
        return;
    }
    ...
    // if (buf->lruPrev == LRU_NOT_IN_LIST) {
    //     return;
    // }
    ...
    // /* unlink from prev */
    // if (buf->lruNext == LRUPREV_HEAD_OF_LIST)
    //     StrategyControl->lruHead = buf->lruPrev;
    // else
    //     (BufferDescriptors + buf->lruNext)->lruPrev = buf->lruPrev;

    // /* unlink from next */
    // if (buf->lruPrev == LRUNEXT_END_OF_LIST)
    //     StrategyControl->lruTail = buf->lruNext;
    // else
    //     (BufferDescriptors + buf->lruPrev)->lruNext = buf->lruNext;
...
StrategyGetBuffer(BufferAccessStrategy strategy, bool *lock_held)
{
	...
	*lock_held = true;
	...
	for (;;)
	{
		 ...
		 */
		buf = strategy == NULL
				? &BufferDescriptors[StrategyControl->lruHead]
...
StrategyInitialize(bool init)
{
	...
	if (!found)
	{
		...
		/* CS460: init LRU list to be empty */
