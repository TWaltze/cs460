...
UnpinBuffer(volatile BufferDesc *buf, bool fixOwner)
{
	...
	if (PrivateRefCount[b] == 0)
	{
		...
		// CS460: link it into LRU list
		if (buf->refcount == 0)
			StrategyLRUEnqueue(buf);
...
void ShowBufferDesc(volatile BufferDesc *buf, int mark)
{
	if (buf == NULL)
	{
		printf("[xx] null BufferDesc pointer\n");
		return;
	}

	if (NBuffers > 128)
	{
		//elog(LOG, "too many buffers: %d with BLCKSZ %d", NBuffers, BLCKSZ);
		return;
	}

	// printf("[%02d] usage_count=%d, refcount=%d, freeNext=%d, dirty=%d, pinned=%d\n",
	// 	buf->buf_id,
	// 	buf->usage_count,
	// 	buf->refcount,
	// 	buf->freeNext,
	// 	buf->flags & BM_DIRTY,
	// 	BufferIsPinned(buf->buf_id));

	// mark: 0 normal, 1 chosen

	char *format = BufferIsPinned(buf->buf_id) ? "<U%d,R%d>" : "[U%d,R%d]";
	switch (mark)
	{
		case 0: printf(format, buf->usage_count, buf->refcount, buf->freeNext); break;
		case 1: printf("[  X  ]"); break;
	}

}

void ShowAllBufferDesc(int chosen)
{
	if (BufferDescriptors == NULL)
	{
		printf("BufferDescriptors == NULL\n");
		return;
	}
	...
	for (; i < NBuffers; i++)
	{
		ShowBufferDesc(&BufferDescriptors[i], i == chosen ? 1 : 0);
	}
}
